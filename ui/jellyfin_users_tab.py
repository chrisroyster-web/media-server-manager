# ui/jellyfin_users_tab.py
"""
Jellyfin User Management tab.
...
def _jf_request(host, port, apikey, method, path, body=None):
...
class JellyfinUsersTab(tk.Frame):
    """Manage Jellyfin users — list, enable/disable, bitrate cap, create, delete."""
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="JELLYFIN  —  USERS",
        ...
        style = ttk.Style()
        style.configure("JFUsers.Treeview",
                        background=t.card_bg,
                        foreground=t.text,
                        fieldbackground=t.card_bg,
                        rowheight=28,
                        font=("Segoe UI", 10))
        style.configure("JFUsers.Treeview.Heading",
                        background=t.surface_dark,
                        foreground=t.text_muted,
                        font=("Segoe UI", 9, "bold"))
        style.map("JFUsers.Treeview",
                  background=[("selected", t.blue)],
                  foreground=[("selected", "#fff")])

        self._tree = ttk.Treeview(tree_frame, columns=cols, show="headings",
                                   style="JFUsers.Treeview", selectmode="browse")
        ...
        # Action panel (shown only when a user is selected)
    ...
    def _do_fetch(self):
        ...
        if not apikey:
            self.after(0, lambda: self._show_status(
                "No Jellyfin API key configured.", self.theme.status_stopped))
            self.after(0, lambda: self._refresh_btn.config(state="normal", text="⟳ Refresh"))
            return
        try:
            users = _jf_request(host, port, apikey, "GET", "/Users")
    ...
    def _toggle_enabled(self):
        ...
        def worker():
            try:
                cfg    = self.controller.config_manager
                _jf_request(cfg.jellyfin_host, cfg.jellyfin_port, cfg.jellyfin_apikey,
    ...
    def _new_user_dialog(self):
        t   = self.theme
        dlg = tk.Toplevel(self)
        dlg.title("Create Jellyfin User")
        dlg.configure(bg=t.bg)
        dlg.resizable(False, False)
        dlg.grab_set()
        dlg.transient(self)

        tk.Label(dlg, text="New Jellyfin User",
