# ui/jellyseerr_tab.py
"""
Jellyseerr request management tab.
Same API as Overseerr (fork) — shows pending/available requests,
recent activity, and top requestors.
"""
...
class JellyseerrTab(tk.Frame):

    def __init__(self, parent, controller):
        super().__init__(parent, bg=controller.theme.bg)
        self.controller = controller
        self.theme      = controller.theme
        self._name      = "Jellyseerr"
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        self._rc = RefreshControl(hdr, self.controller, "jellyseerr",
        ...
        nb_style = ttk.Style()
        nb_style.configure("JS.TNotebook", background=t.bg, borderwidth=0)
        nb_style.configure("JS.TNotebook.Tab",
                           background=t.surface, foreground=t.text_muted,
                           padding=[12, 6], font=t.font_small)
        nb_style.map("JS.TNotebook.Tab",
                     background=[("selected", t.surface_light)],
                     foreground=[("selected", t.text)])

        self._nb = ttk.Notebook(self, style="JS.TNotebook")
    ...
    def _fetch(self):
        try:
            cfg  = self.controller.config_manager
            host = cfg.jellyseerr_host
            port = cfg.jellyseerr_port
            key  = cfg.jellyseerr_apikey
